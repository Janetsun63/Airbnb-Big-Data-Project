    ...
    for itr in itrs:
        for rank in ranks:
            for reg_parameter in reg_parameters:
                ...
                print ('For iteration %s, rank %s and regularization parameter %s the RMSE is %s' % (itr,rank, reg_parameter, error))
                if error < min_error:
                    min_error = error
                    best_itr = itr
                    best_rank = rank
                    best_regularizer = reg_parameter
    print ('The best model was trained with iteration %s rank %s, regularization %s and minimum RMSE %s' % (best_itr,best_rank,best_regularizer,min_error))

    #The best model was trained with iteration 20 rank 5, regularization 0.1 and minimum RMSE 1.301914191438507
    als = ALS(
            maxIter= best_itr,
